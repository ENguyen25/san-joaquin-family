...
function App() {
  ...
  const [renderList, renderEventsList] = useState('');
  ...
  const getEvents = () => {
    axios.get('http://localhost:5000/event/')
      ...
      .then(data => {
        localStorage.setItem('renderList', JSON.stringify(data))
        return data
      })
      .then(data => {
        renderEventsList(JSON.parse(localStorage.getItem('renderList')))
        return data
      })
      .catch(() => {
        console.log('Error')
  ...
  function findEvents(organizer) {
    fetch(`https://www.eventbriteapi.com/v3/organizers/${organizer}/events/?start_date.range_start=2022-03-23T00:00:00Z&token=TDI3RNBKVRWFKWHZ4IO6`)
  ...
  return (
    <div className="App">
      ...
      {
        renderList && <Events data={renderList} />
      }
    </div>
  );
}
